        ...
        public async Task<IActionResult> AddEdit(Product product, int[] ingredientIds, int catId)
        {
            ...
            if (ModelState.IsValid)
            {
                if (product.ImageFile != null)
                {
                    ...
                    if (!supportedTypes.Contains(product.ImageFile.ContentType))
                    {
                        ModelState.AddModelError("", "Fisierul trebuie sa fie o imagine (.jpg, .png)");
                        return View(product);
                    }
                    string uploadsFolder = Path.Combine(_webHostEnvironment.WebRootPath, "images");
                    string uniqueFileName = Guid.NewGuid().ToString() + "_" + product.ImageFile.Name;
